...
The briefing agent must not rewrite the source document as a whole. The host application owns parsing, replacement boundaries, and reconstruction of the generated briefing.
...
1. **Source Template** — editable by the user and retains the original Markdown, macros, entity references, and `[[...]]` instructions for future runs.
...
The exact resolution stage for deterministic macros is an implementation detail as long as the resulting Conversation context uses the intended resolved value and macro processing does not permit the briefing agent to rewrite surrounding user-authored text.

### 4.2 Self-replacing agent instructions — `[[...]]`
...
An editable Markdown-style text area containing the user's briefing structure, deterministic macros, self-replacing agent instructions, and entity references.
...
- deterministic macro resolution stage;
