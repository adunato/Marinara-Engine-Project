...
| CR015 | merged into main | Daily Conversation Memories | Adds a built-in agent and Conversation UI for day-scoped memories that complement summaries and can be used with or instead of current memory recall. | None | Implemented in `5ff1fbcb` with participant-ID validation in `8da9b278`; focused API/UI Playwright validation passed (3 tests, including direct Agent Settings coverage), server and changed-client checks passed, and the primary checkout production build passed. |
