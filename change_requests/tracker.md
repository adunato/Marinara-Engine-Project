...
| CR013 | standalone | Scene Conversation Context Continuity | Gives `/scene` planning and the created roleplay the same summary-rich history and full current-day context available to normal Conversation generation. | None | Implemented in `d62923e8` on `change/CR013-scene-conversation-context`; dedicated regression and server-only TypeScript validation passed. |
