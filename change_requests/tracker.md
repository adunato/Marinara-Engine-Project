...
| CR014 | merged into main | Full Cross-Chat Conversation Context | Gives a Conversation complete summaries and visible transcripts from enabled Conversations sharing any current character, with explicit source and speaker attribution. | None | Implemented in `8588a307`; shared-character boundary restored in `d1c7cb1a`, validated, built in the primary checkout, and fast-forward merged into local application `main`. |
