...
| CR015 | merged into main | Daily Conversation Memories | Adds a built-in agent and Conversation UI for day-scoped memories that complement summaries and can be used with or instead of current memory recall. | None | Implemented in `5ff1fbcb`, participant-ID validation in `8da9b278`, Conversation picker exposure in `3dd20d49`, and UI/configuration refinement in `15f12f2b`; focused API/UI Playwright validation passed (4 tests, including Conversation configuration, compact/themed editor presentation, and animated generation progress), server and changed-client checks passed, and the primary checkout production build passed. |
