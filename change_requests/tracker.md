...
| CR013 | merged into main | Scene Conversation Context Continuity | Gives `/scene` planning and the created roleplay the same summary-rich history and full current-day context available to normal Conversation generation. | None | Implemented in `d62923e8` and fast-forwarded into local application `main`; dedicated regression and server-only TypeScript validation passed. |
