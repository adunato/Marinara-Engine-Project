...
| CR014 | merged into main | Full Cross-Chat Conversation Context | Gives every character complete summaries and visible transcripts from every other cross-chat-enabled Conversation, with explicit source and speaker attribution. | None | Implemented and fast-forward merged into local application `main` in `8588a307`; focused regression and server TypeScript validation passed. |
