...
description: Create or update Marinara-Engine fork pull-request description artifacts in change_requests/CRXXX_*/PR.md. Use when Codex needs to draft, fill out, or refresh a PR description for an Adunato fork change request, including why, what changed, validation, docs/release impact, and UI evidence.
---

# Marinara Fork PR Description

## Purpose

Use this skill to prepare a reviewer-ready PR description for a change request being reviewed within the maintained Adunato fork and store it with the change-request artifacts. Upstream Pasta-Devs contribution is outside the active workflow.
...
2. Determine the change request ID:
   ...
   - Otherwise parse the current branch when it starts with `change/CRXXX-` or otherwise clearly identifies the CR.
...
5. Gather source material before writing:
   - Read the CR HLD/IP, `ALIGNMENT.md` when present, and any existing PR draft in the CR folder.
   - Inspect parent `git status --short --branch` and nested app `git -C Marinara-Engine status --short --branch`.
   - Inspect the relevant nested app branch diff and commit log against the intended fork baseline when available.
